---
title: "Hep B and C Report"
author: ""
date: "Report date: `r format(Sys.Date(), '%d %b %Y')`"
output: html_document
knit: (function(input, ...) {
  rmarkdown::render(input, output_file = paste0("HEPs Report", "-", format(Sys.Date(), "%d %b %Y")), output_dir = "../Output") })
---

<!--
Things to consider for analysis:
Due to smaller numbers than STI, change per population to 1 million
Alternatively, instead of monthly rates, we could consider quarterly rates or yearly rates?
Need to also look at risk factors and treatment or care if data allows


Version control notes:
- included cumulative graph function
- Hep B and Hep C sections

-->

```{r setup, include=FALSE}
# ensure to change the token argument in the dbConnect() to either "PHAR" if working from NEPHU office or "PHAR proxy" if working from home
...
here::i_am("Heps-Report-Code.Rmd")
...
```{r extract_data}
hep_raw <- DBI::dbGetQuery(con, 
                           "
                           SELECT * 
                           FROM dh_public_health.phess_release.caseevents ce 
                           LEFT JOIN dh_public_health.phess_release.events_hbv_hcv ev ON ce.EVENT_ID = ev.EVENT_ID 
                           LEFT JOIN dh_public_health.phess_release.events_hbv_hcv_risks rk ON ce.EVENT_ID = rk.EVENT_ID 
                           WHERE CONDITION_TYPE = 'Blood Borne Viruses' 
                           AND EVENT_DATE BETWEEN '2022-01-01' AND '2025-08-31' 
...
hepc <- c("Hepatitis C - <24 months of age", "Hepatitis C - Newly acquired", "Hepatitis C - Unspecified")

hepbc <- c(hepb, hepc)

# epimonth_breaks <- c(seq.Date(from=min(hep_data$mth_yr_fmt), to=max(hep_data$mth_yr_fmt) + months(1), by="month"))
...
hep_data <- hep_raw %>% 
  ...
  #        meets_invehepgation_criteria, invehepgation_status, case_found_by) %>% 
  filter(defn=="Confirmed" | defn=="Probable") %>% 
  filter(event_type=="Case") %>% 
  # filter condition to hepB and hepC
  filter(condition %in% hepbc) %>% 
  ...
  mutate(agecat5 = case_when(between(age, 0, 4) ~ "0-4", 
         ...
         disease = case_when(condition %in% hepb ~ "HepB", 
                          condition %in% hepc ~ "HepC"), 
...
nephu_data <- filter(hep_data, lga %in% nephu_lgas)
...
The diseases covered in this report are Hepatitis B and Hepatitis C.
...
f.agesex_pyr_nephu <- function(std) {
  pyramid.dat <- hep_data %>% 
    filter(disease==std) %>% 
    filter(lga %in% nephu_lgas) %>% 
    select(age, agecat5, sex, defn, condition, disease) %>% 
    filter(sex %in% c("Male", "Female")) %>% 
    mutate(sex=fct_rev(fct_drop(sex)))
  
  sex_other_pct <- hep_data %>% 
    filter(disease==std) %>% 
    filter(lga %in% nephu_lgas) %>% 
    tabyl(sex) %>% 
  ...
  sex_other_sentence <- if(sex_other_pct==0) {""}
  else{paste0("Excludes sex=Not stated / Other which makes up ", sex_other_pct, "% of total cases")}
...
```{r, eval=FALSE}
f.agesex_pyr_nephu("HepC")

```

```{r vic rates}
hep_vic_mth_count <- hep_data %>% 
  #filter(condition %in% main_hep) %>% 
  group_by(mth_yr_fmt, disease) %>% 
  summarise(count = n(), cond = first(condition)) %>% 
  mutate(lga = "VIC")

hep_vic_mth_rate <- hep_vic_mth_count %>% 
  left_join(., pop_table, by="lga") %>% 
  mutate(mth_rate = count/pop_2021*1000000)
...
f.trend_plot_nephu <- function(std) {
  nephu_mth_count <- hep_data %>% 
    filter(disease==std) %>% 
    filter(lga %in% nephu_lgas) %>% 
    group_by(mth_yr_fmt) %>% 
    summarise(count=n(), cond=first(condition), disease=first(disease)) %>% 
    mutate(lga="NEPHU")
  
  nephu_mth_rate <- left_join(nephu_mth_count, pop_table, by="lga") %>% 
    mutate(mth_rate = count/pop_2021*1000000)
  
  vic_mth_rate <- filter(hep_vic_mth_rate, disease==std) %>% rename(state=lga)
  
  plot <- ggplot(nephu_mth_rate) + 
    geom_line(aes(x=mth_yr_fmt, y=mth_rate, group=disease, col="hep")) + 
    geom_line(data=vic_mth_rate, aes(x=mth_yr_fmt, y=mth_rate,  group=disease, col="hep", lty="VIC")) + 
    ...
    #scale_colour_manual(values=c("hep"="red", "trend"="slateblue"), labels=c("hep"=paste0(std, " (NEPHU)"), "trend"="NEPHU trend line"), name="") + 
    scale_colour_manual(values=c("hep"="red", "trend"="slateblue"), labels=c("hep"="NEPHU", "trend"="NEPHU trend and \n95% CI"), name="") + 
    ...
    labs(title=paste0("Monthly ", std, " rate per 1,000,000 population\n(NEPHU compared to VIC)"), 
         caption=paste0("Data period: ", start_date, " to ", max_date))
...
f.trend_plot_lga <- function(std) {
  ...
  plot <- ggplot(lga_mth_rate) + 
    ...
    scale_x_date(expand=c(0,0), date_breaks="3 months", date_minor_breaks="month", date_labels="%b %Y", name="") + 
    scale_y_continuous(expand=c(0,0), breaks=integer_breaks(), name="Monthly notified rate \n(per 1,000,000 population)") + 
    scale_colour_manual(values=c("hep"="red", "trend"="blue"), labels=c("hep"="LGA",  "trend"="LGA trend and \n95% CI"), name="") + 
...
```{r}
f.trend_plot_lga("HepB")

```

```{r cum_yr_graph function}
# Create function to plot cumulative case number - NEPHU level

f.cum_plot_nephu <- function(std) {
  cum_data <- hep_data %>% 
    filter(disease==std) %>% 
    filter(lga %in% nephu_lgas) %>% 
    group_by(yr, epimonth) %>% 
    summarise(count=n(), cond=first(condition), disease=first(disease)) %>% 
    mutate(lga="NEPHU") %>% 
  ...
  plot <- ggplot(cum_data) + 
    geom_line(aes(x=epimonth, y=cum_count, group=factor(yr), col=factor(yr))) + 
    #scale_x_discrete(expand=c(0,0)) + 
    scale_y_continuous(breaks=seq(0, 350, by=50)) + 
    cowplot::theme_cowplot() + 
    theme(plot.title = element_text(size=12)) + 
    labs(title=paste0("Cumulative ", std, " case numbers by month in NEPHU"), 
         caption=paste0("Data period: ", start_date, " to ", max_date), 
         x="", 
...
```{r}
f.cum_plot_nephu("HepB")

```

```{r risk function}

# hep_exp.dat <- left_join(nephu_data, exp_data, by="event_id") %>% 
#   filter(event_date>=min_date_1yr) %>% 
...
f.sex_exp_table <- function(std) {
  table <- tabyl(hep_exp.dat %>% filter(disease==std), lga, sexual_exp) %>% 
...
tabyl(nephu_data[nephu_data$disease=="HepB", ], atsi)

# country of_birth distribution
cob.num <- xtabs(~aus_born, data=cond_data)
cob.pct <- percent(prop.table(cob.num))
tabyl(nephu_data[nephu_data$disease=="HepB", ], aus_born)

```

``` {r hepb_section1}
hepb_agesex <- f.agesex_pyr_nephu("HepB")

hepb_nephu <- f.trend_plot_nephu("HepB")

ggpubr::ggarrange(hepb_agesex, hepb_nephu, nrow=1)
...
##### Monthly Hepatitis B rate per 1,000,000 population compared to VIC average by LGA
...
```{r}
hepc_agesex <- f.agesex_pyr_nephu("HepC")

hepc_nephu <- f.trend_plot_nephu("HepC")

ggpubr::ggarrange(hepc_agesex, hepc_nephu, nrow=1)
...
##### Monthly Hepatitis C rate per 1,000,000 population compared to VIC average by LGA

```{r}
f.trend_plot_lga("HepC")
```
<br><br>
...
vic_mth_count_hiv <- filter(hep_vic_mth_count, disease=="HIV") %>% 
  select(-lga)
  
hiv_nephu_map <- ggplot(nephu_mth_count_hiv) + 
    geom_line(aes(x=mth_yr_fmt, y=count, group=disease, col="hep")) + 
    geom_line(data=vic_mth_count_hiv, aes(x=mth_yr_fmt, y=count,  group=disease, col="hep", lty="Vic Average")) + 
    ...
    scale_colour_manual(values=c("hep"="red", "trend"="slateblue"), labels=c("hep"="HIV", "trend"="trend line"), name="") + 
