...
* Map showing annual rate per 100,000 population at LGA level
...
```{r risk function}
sexual_exp_levels <- c("Person(s) of same sex only", "Person(s) of opposite sex only", "Person(s) of both sexes", "Other", "Unknown")
...
```{r}
f.rate_map_lga("Gonorrhea") + labs(caption = "grey area denote no notified case in the last 12 months")
...
flextable(hiv_counts_lga) %>% set_header_labels(lga="LGA") %>% autofit() %>% 
  add_footer_lines(paste0("Note: Yr2024 is not a full year, ends ", format(max_date, '%d %b %Y'))) %>% align(i=1, j=1, align="right", part="footer")
